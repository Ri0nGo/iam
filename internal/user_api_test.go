...
func TestUserAPI(t *testing.T) {
	...
	createResp := s.doJSON(http.MethodPost, "/api/iam/users", map[string]any{
		"username":     "alice",
		"password":     "123456",
		"display_name": "Alice",
	...
	detailResp := s.doJSON(http.MethodGet, fmt.Sprintf("/api/iam/users/%d", createdUser.ID), nil, admin.AccessToken)
	...
	bindResp := s.doJSON(http.MethodPut, fmt.Sprintf("/api/iam/users/%d/roles", createdUser.ID), map[string]any{"role_codes": []string{"admin", "operator"}}, admin.AccessToken)
	if bindResp.Code != http.StatusOK {
		t.Fatalf("bind user roles status=%d body=%s", bindResp.Code, bindResp.Body.String())
	}

	statusResp := s.doJSON(http.MethodPut, fmt.Sprintf("/api/iam/users/%d/status", createdUser.ID), map[string]any{"status": 2}, admin.AccessToken)
	if statusResp.Code != http.StatusOK {
		t.Fatalf("update user status=%d body=%s", statusResp.Code, statusResp.Body.String())
	}

	passwordResp := s.doJSON(http.MethodPut, fmt.Sprintf("/api/iam/users/%d/password", createdUser.ID), map[string]any{"password": "new-password"}, admin.AccessToken)
	if passwordResp.Code != http.StatusOK {
		t.Fatalf("reset password status=%d body=%s", passwordResp.Code, passwordResp.Body.String())
	}

	disabledLoginResp := s.doJSON(http.MethodPost, "/api/iam/auth/login", map[string]any{"username": "alice", "password": "new-password"}, "")
